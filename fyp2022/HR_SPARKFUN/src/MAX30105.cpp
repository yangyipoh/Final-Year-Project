...
void MAX30102::softReset(void) {
  ...
  // Timeout after 100ms
  unsigned long startTime = gpioTick();
  while (gpioTick() - startTime < 100000)
  {
    uint8_t response = readRegister8(_i2caddr, MAX30102_MODECONFIG);
    if ((response & MAX30102_RESET) == 0) break; //We're done!
    gpioSleep(0,0,1000); //Let's not over burden the I2C bus //delay for 1ms
  }
}
...
float MAX30102::readTemperature() {
  ...
  while (gpioTick() - startTime < 100000)
  {
    ...
    gpioSleep(0,0,1000); //Let's not over burden the I2C bus
  }
...
readRegister8(_i2caddr, MAX30102_PARTICLECONFIG);
  ...
  if (readPointer != writePointer)
  {
    ...
    while (bytesLeftToRead > 0)
    {
      ...
      while (toGet > 0)
      {
        ...
        //byte temp[sizeof(uint32_t)]; //Array of 4 bytes that we will convert into long -original
	int num = activeLEDs * 3;
	char buf[(char) num]; //buffer to contain data from sensor (3 bytes per LED)
	char temp[12]; //Array of 12 bytes that will be used to store the data from the buffer with the padded zeros
        uint32_t tempLong=0;

	//Commands to do block reading of the sensor using repeated starts. Read data from all LEDs at once.
	char cmdBuf[ ]=	{4, MAX30102_ADDRESS, //set chip address
			    	 	2,  //start
				 	7, 1, MAX30102_FIFODATA, //write 1 byte that is the register address that will be read
				 	2, 6, (char) num, //repeated start, read a block of bytes for all LEDs from the register
				 	3, //stop
				 	0 //end of commands
				 	};

        //Burst read nine bytes - RED, IR, GREEN
        bbI2CZip(27, cmdBuf, sizeof(cmdBuf), buf, sizeof(buf));

	//Read back into temp array (goes GREEN, IR, RED). The sensor reads from MSB to LSB but memcpy flips it (LSB to MSB). Need to read into this order so that memcpy can read it properly. 
	int i,j;
	j=0;
	for (i = 0; i<12;i++){
		if((i==3)||(i==7)||(i==11)){ //if it is the fourth part of the 32 bit number aka temp[3], make it zero or else there will be a random number screwing things up
			temp[i]=0;
			if(i==3){
				j=3;		
			}
	...
	//RED
	memcpy(&tempLong, &temp[8], 4);	//Convert array to long //  //memcpy(&tempLong, temp, sizeof(tempLong)) -original;
	...
	//IR LED
       if (activeLEDs > 1)
        {
	memcpy(&tempLong, &temp[4], 4); //Convert array to long
	tempLong &= 0x3FFFF; //Zero out all but 18 bits
...
{
  uint32_t markTime = gpioTick();
  
  while(1)
  {
	if(gpioTick() - markTime > maxTimeToCheck*1000) return(false);
...
/*  _i2cPort->beginTransmission(address);
  ...
  char cmdBuf[ ]=	{4, address, //set chip address
			    	 2,  //start
				 7, 1, reg, //write 1 byte that is the register address that will be read
				 2, 6, 1, //repeated start, read 1 byte from the register
				 3, //stop
				 0 //end of commands
				 };
...
char cmdBuf[ ]=	{4, address, //set chip address
			    	 2,  //start
				 7, 2, reg, value, //write 2 bytes: register address to be written to and the data
				 3, //stop
				 0 //end of commands
				 };
bbI2CZip(27, cmdBuf, sizeof(cmdBuf), NULL, 0);
}
